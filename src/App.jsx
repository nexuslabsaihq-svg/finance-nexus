...
function App() {
  ...
  if (!isAppView) {
    return (
      <>
        <Landing onLogin={loginWithGoogle} authUser={authUser} authError={authError} />
        <CookieConsent />
      </>
    );
  }
      ...
      <LockScreen>
        ...
        <div className="app">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', width: '100%', height: '100vh', overflow: 'hidden' }}>
            <Header />
            <main className="main" style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
              <div style={{ flex: 1 }}>
                {renderPage()}
              </div>
              <AppFooter />
