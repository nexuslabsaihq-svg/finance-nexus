...
export default function Sidebar() {
  ...
  const activeDebts = deudas.filter((d) => toAmount(d.balance ?? d.monto) > 0);
  const upcomingDebtCount = activeDebts.filter((d) => periodPrefix && d.vencimiento?.startsWith(periodPrefix)).length;
  ...
  return (
    <nav className="sidebar">
      ...
      <NavItem id="deudas" icon="📋" label="Deudas" badge={activeDebts.length || null} activePage={activePage} setActivePage={setActivePage} />
