...
export default function IA() {
  const { chatsIA, setChatsIA, ingresos: _ingresos, gastos: _gastos, ahorros: _ahorros, deudas: _deudas, bancos: _bancos, inversiones: _inversiones, authUser } = useAppData();
                 ...
                 <div style={{
                   ...
                   <div style={{whiteSpace: 'pre-wrap'}}>
                     {msg.content?.split(/(\*[^*]+\*)/g).map((part, i) =>
                       i % 2 === 1 ? <b key={i}>{part.slice(1, -1)}</b> : part
                     )}
                   </div>
                 </div>
