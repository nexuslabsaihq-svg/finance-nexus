      ...
      <div style={{ maxWidth:1200, margin:"0 auto" }}>
          ...
          {[
            {
              title: "Productos",
              links: [
                { label: "Finance Nexus App", href: "#app" },
                { label: "Módulo Contabilidad", href: "#productos" },
                { label: "Networking Hub", href: "#networking" },
                { label: "Enterprise", href: "#precios" }
              ]
            },
            {
              title: "Empresa",
            ...
            },
            {
              ...
              links: [
                { label: "📜 Términos de uso (Borrador)", href: "/legal/terminos-y-condiciones.html", external: true },
                { label: "🔒 Privacidad (Ley 21.719)", href: "/legal/politica-privacidad.html", external: true },
                { label: "✉️ Soporte y Reclamos", href: "mailto:nexuslabsai.hq@gmail.com", external: true },
                { label: "🏛️ Portal SERNAC (Externo)", href: "https://www.sernac.cl/portal/618/w3-propertyvalue-59368.html", external: true },
                { label: "🍪 Configurar Cookies", onClick: () => window.dispatchEvent(new Event('fn_open_cookie_consent')) }
              ]
            },
          ].map((col, i) => (
            <div key={i}>
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:12 }}>
          <div style={{ fontSize:12, color:"rgba(255,255,255,0.25)" }}>
            © {year} Finance Nexus SpA (RUT pendiente de confirmación) · Santiago, Chile · Todos los derechos reservados
          </div>
...
/* ─── APP ROOT ───────────────────────────────────────────────────────────── */
export default function Landing({ onLogin, authUser, authError }) {
  ...
  const handleAccess = async () => {
    ...
    try {
      ...
      let msg = e?.message || "Error al iniciar sesión con Google";
      if (msg.includes("auth/unauthorized-domain")) {
        msg = "Dominio no autorizado en Firebase Console. Agrega tu dominio de Vercel en Firebase Console > Authentication > Settings > Authorized domains.";
      } else if (msg.includes("auth/popup-closed-by-user")) {
        msg = "La ventana de inicio de sesión fue cerrada antes de completar.";
      } else if (msg.includes("auth/popup-blocked")) {
        msg = "El navegador bloqueó la ventana emergente. Por favor, permite ventanas emergentes para este sitio.";
      } else if (msg.includes("Firebase no está inicializado")) {
        msg = "Variables de entorno ausentes en Vercel. Configura las variables VITE_FIREBASE_* en Project Settings > Environment Variables.";
      }
      setLocalError(msg);
    } finally {
  ...
  return (
    <>
      ...
      <div id="landing-scroll-container" style={{ height:"100vh", overflowY:"auto", overflowX:"hidden", scrollBehavior:"smooth", position:"relative" }}>
        ...
        <Strip />
        <Productos />
        <AppSection onOpenLogin={openLogin} />
        <Contabilidad />
        <Networking />
        <Precios />
        <Testimonios />
      ...
      {showLogin && (
        <div style={{ position:"fixed", inset:0, zIndex:9999, display:"flex", alignItems:"center", justifyContent:"center", background:"rgba(10,13,20,0.85)", backdropFilter:"blur(16px)", animation:"fadeIn .3s ease" }}>
          <div style={{ background:"linear-gradient(160deg, #181820, #0f0f14)", border:"1px solid rgba(255,140,66,0.3)", borderRadius:24, padding:"48px 40px", width:"90%", maxWidth:420, textAlign:"center", position:"relative", boxShadow:"0 32px 64px rgba(0,0,0,0.5)" }}>
            ...
            <p style={{ fontSize:15, color:"rgba(255,255,255,0.5)", marginBottom: displayError ? 16 : 32, lineHeight:1.6 }}>Ingresa de forma segura para acceder al entorno administrativo de Finance Nexus.</p>
            ...
            {displayError && (
              <div style={{
                ...
                <div style={{ fontWeight: 700, marginBottom: 4 }}>⚠️ Error de inicio de sesión:</div>
            ...
            <button 
              ...
              style={{ 
                ...
                padding:"16px", 
                borderRadius:14, 
                background: isLoggingIn ? "rgba(255,255,255,0.6)" : "#fff", 
                color:"#0f0f14", 
                fontSize:16, 
                fontWeight:700, 
                border:"none", 
              ...
              onMouseOver={e=>{if(!isLoggingIn){e.currentTarget.style.transform="translateY(-2px)"; e.currentTarget.style.boxShadow="0 12px 24px rgba(255,255,255,0.2)"}}} 
              onMouseOut={e=>{if(!isLoggingIn){e.currentTarget.style.transform="translateY(0)"; e.currentTarget.style.boxShadow="0 8px 16px rgba(255,255,255,0.1)"}}}
            >
              {isLoggingIn ? (
