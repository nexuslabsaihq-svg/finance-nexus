...
export default function Gastos() {
  const { gastos, setGastos, presupuestos, configuracion, bancos, setActivePage } = useAppData();
  ...
  const desglose = presupuestos.map(p => {
    ...
    const rawPct = p.monto ? Math.round((actual / p.monto) * 100) : 0;
    const pct = Math.min(100, rawPct);
    const isExceeded = actual > p.monto;
    return { ...p, actual, pct, rawPct, isExceeded };
  });

  const categoriasExcedidas = desglose.filter(d => d.isExceeded);
  ...
  return (
    <div className="page active" style={{ display: 'flex' }}>
      ...
      {categoriasExcedidas.length > 0 && (
        <div className="alert al-p" style={{ borderColor: 'var(--pink)', background: 'rgba(232, 93, 117, 0.1)' }}>
          <span className="al-icon">🚨</span>
          <div className="al-body">
            <div className="al-title" style={{ color: 'var(--pink)' }}>Presupuesto Excedido</div>
            {categoriasExcedidas.map(c => c.cat).join(', ')} ha(n) superado el límite fijado. Ajusta tus egresos o actualiza los topes en Presupuestos.
          </div>
        </div>
      )}
      {pctUso > 80 && (
      ...
      <div className="g2">
        <div className="card">
          <div className="card-hdr" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <button className="btn btn-gh btn-sm" onClick={() => setActivePage('presupuestos')}>
          ...
          <div className="tw"><table><thead><tr><th>Categoría</th><th className="r">Presupuesto</th><th className="r">Actual</th><th>Uso</th></tr></thead><tbody>
            {desglose.length === 0 && (
              <tr>
                <td colSpan="4" style={{ textAlign: 'center', padding: '24px', color: 'var(--text2)' }}>
                  <div style={{ marginBottom: '8px' }}>No hay presupuestos definidos para este período.</div>
                  <button className="btn btn-p btn-sm" onClick={() => setActivePage('presupuestos')}>
            ...
            {desglose.map((d, i) => (
              <tr key={i}>
                <td className="tdp">{d.cat}</td>
                <td className="tdr tdm">${d.monto.toLocaleString()}</td>
                <td className={`tdr ${d.isExceeded ? 'neg' : ''}`}>${d.actual.toLocaleString()}</td>
                <td>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <div style={{ width: '80px' }}>
                      <div className="pt">
                        <div
                          ...
                          style={{
                            width: `${d.pct}%`,
                            background: d.isExceeded ? 'var(--pink)' : d.pct > 80 ? 'var(--orange)' : 'var(--green)'
                          }}
                        ></div>
                      </div>
                    </div>
                    <span style={{
                      fontSize: '11.5px',
                      color: d.isExceeded ? 'var(--pink)' : d.pct > 80 ? 'var(--orange)' : 'var(--green)',
                      fontWeight: '700'
                    }}>
                      {d.rawPct}%
                    </span>
