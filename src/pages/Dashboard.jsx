...
import { GeminiKey } from '../firebase/config';
...
export default function Dashboard({ period }) {
  ...
  useEffect(() => {
    if (!GeminiKey || (!ingresos.length && !gastos.length)) {
      setLoadingAi(false);
      return;
    }
  ...
  // Cálculo de cuotas y compromisos de deuda del mes
  const pagoDeudasMes = deudas.reduce((sum, debt) => {
    const mensual = toAmount(debt.pagoMensual);
    if (mensual > 0) return sum + mensual;
    if (debt.vencimiento && prefix && debt.vencimiento.startsWith(prefix)) {
      return sum + toAmount(debt.balance ?? debt.monto);
    }
    return sum;
  }, 0);

  // Score Financiero Real (Fórmula Opción B: Solvencia neta sobre ingresos)
  const score = totalIngresos > 0
    ? Math.max(0, Math.min(1000, Math.round(((totalIngresos - totalGastos - pagoDeudasMes) / totalIngresos) * 1000)))
    : 0;

  let scoreColor = 'var(--pink)';
  let scoreLabel = 'Crítico';
  let scoreDesc = 'Tus gastos y compromisos superan tus ingresos. Se recomienda reducir egresos inmediatos.';
  if (score >= 700) {
    scoreColor = 'var(--green)';
    scoreLabel = 'Excelente';
    scoreDesc = 'Capacidad sobresaliente de ahorro y solvencia operativa neta.';
  } else if (score >= 400) {
    scoreColor = 'var(--orange)';
    scoreLabel = 'Moderado';
    scoreDesc = 'Salud financiera regular. Controla gastos discrecionales y cuotas de crédito.';
  }

  // Comparativa con el mes anterior
  const prevPrefixes = getPreviousPeriodPrefixes(period, 2);
  const prevMonthPrefix = prevPrefixes.length >= 2 ? prevPrefixes[prevPrefixes.length - 2]?.prefix : null;
  const prevIngresos = prevMonthPrefix ? filterByPeriod(ingresos, prevMonthPrefix).reduce((sum, i) => sum + toAmount(i.monto), 0) : 0;
  const prevGastos = prevMonthPrefix ? filterByPeriod(gastos, prevMonthPrefix).reduce((sum, g) => sum + toAmount(g.monto), 0) : 0;

  const getComparison = (current, prev) => {
    if (!prev || prev === 0) return 'Primer registro';
    const diff = current - prev;
    const pct = Math.round((diff / prev) * 100);
    return `${pct >= 0 ? '+' : ''}${pct}% vs mes ant.`;
  };
  ...
  return (
    <div className="page active" style={{ display: 'flex' }}>
      ...
      <div className="gms">
        <div className="g4">
          <Metric label="Ingresos del mes" value={formatCurrency(totalIngresos)} icon="💰" color="var(--orange)" detail={`${getComparison(totalIngresos, prevIngresos)} · ${ingresosMes.length} mov.`} />
          <Metric label="Gastos del mes" value={formatCurrency(totalGastos)} icon="💸" color="var(--pink)" detail={`${getComparison(totalGastos, prevGastos)} · ${gastosMes.length} mov.`} />
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          <div className="card card-glow-b" style={{ textAlign: 'center', padding: '20px 16px' }}>
            <div className="card-hdr" style={{ justifyContent: 'center', marginBottom: '6px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px', margin: '10px 0' }}>
              <div style={{ fontSize: '42px', fontWeight: 900, color: scoreColor, fontFamily: 'var(--mono)', lineHeight: 1 }}>
                {score}
              </div>
              <div style={{ textAlign: 'left' }}>
                <span className="badge" style={{ background: `${scoreColor}22`, color: scoreColor, fontWeight: 700, fontSize: '11px', display: 'inline-block' }}>
                  {scoreLabel}
                </span>
            ...
            <div style={{ fontSize: '12px', color: 'var(--text2)', lineHeight: '1.4', maxWidth: '260px', margin: '0 auto 12px' }}>
              {scoreDesc}
            </div>
            <div style={{ width: '100%', height: '6px', background: 'var(--surface2)', borderRadius: '3px', overflow: 'hidden' }}>
              <div style={{ width: `${(score / 1000) * 100}%`, height: '100%', background: scoreColor, transition: 'width 0.6s ease' }}></div>
            </div>
          </div>
