...
const getClientDevice = () => {
  ...
  let os = 'Dispositivo';
  if (/Windows NT 10.0|Windows NT 11.0/.test(ua)) os = 'Windows 11/10';
...
const useFirestoreState = (key, initialValue, activeUid, isViewer) => {
      ...
      if (snap.exists()) {
        ...
        if (legacy) {
          try {
            ...
            if (!isViewer) setDoc(docRef, { data: initialRef.current }).catch(console.error);
          }
        } else {
          if (!isViewer) setDoc(docRef, { data: initialRef.current }).catch(console.error);
        }
...
export const AppDataProvider = ({ children }) => {
  ...
  const [ingresos, setIngresos] = useFirestoreState('fn_ingresos', [], activeUid, isViewer);

  const [gastos, setGastos] = useFirestoreState('fn_gastos', [], activeUid, isViewer);

  const [transferencias, setTransferencias] = useFirestoreState('fn_transferencias', [], activeUid, isViewer);
  const [bancos, setBancos] = useFirestoreState('fn_bancos', [], activeUid, isViewer);
  const [ahorros, setAhorros] = useFirestoreState('fn_ahorros', [], activeUid, isViewer);
  const [inversiones, setInversiones] = useFirestoreState('fn_inversiones', [], activeUid, isViewer);
  const [deudas, setDeudas] = useFirestoreState('fn_deudas', [], activeUid, isViewer);
  const [presupuestos, setPresupuestos] = useFirestoreState('fn_presupuestos', [], activeUid, isViewer);
  ...
  const [sesionesIA, setSesionesIA] = useFirestoreState('fn_sesiones_ia', [], activeUid, isViewer);
  const [notificaciones, setNotificaciones] = useFirestoreState('fn_notifs', [], activeUid, isViewer);
  ...
  const [sesiones, setSesiones] = useFirestoreState('fn_sesiones', [
    { id: 1, device: getClientDevice(), ip: 'Sesión local activa', location: 'Chile', current: true }
  ], activeUid, isViewer);
